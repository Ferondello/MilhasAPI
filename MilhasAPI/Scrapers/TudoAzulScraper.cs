...
/// <summary>
/// Scraper para o programa TudoAzul (Azul Linhas Aéreas).
/// Coleta cotações da página de compra de pontos: https://www.tudoazul.com.br/compre-pontos
///
...
/// </summary>
public class TudoAzulScraper : BaseMilesScraper
{
    public override string ProgramName => "TudoAzul";
    protected override string TargetUrl => "https://www.tudoazul.com.br/compre-pontos";

    // ── Seletores HTML ──────────────────────────────────────────────
    private const string PriceXPath = "//span[contains(@class,'price-per-point')]";
    private const string BonusXPath = "//span[contains(@class,'bonus-label')]";
    private const string PromoXPath = "//div[contains(@class,'promo-highlight')]";
    // ───────────────────────────────────────────────────────────────

    public TudoAzulScraper(IHttpClientFactory httpClientFactory, ILogger<TudoAzulScraper> logger)
    ...
    protected override async Task<MilesQuote?> ExecuteScrapingAsync(CancellationToken cancellationToken)
    {
        ...
        // Bônus de pontos (ex: "100% de bônus" → multiplier 2.0)
